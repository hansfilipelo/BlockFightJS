...
export class LShape {
  ...
  rotate() {
    if (this.current_rotation_ === ROTATION.DEFAULT) {
      // Rotate to 90° clockwise around stone[1] (4, -2):
      // [X][X][X]
      // [X]
      ...
      // Rotate to 180° clockwise around stone[1] (4, -2):
      // [X][X]
      //    [X]
      //    [X]
      if (this.stones_[0].canMove(-1, 1) &&
      ...
      // Rotate to 270° clockwise around stone[1] (4, -2):
      //       [X]
      // [X][X][X]
      let x_pos = this.stones_[0].x_pos();
      ...
      // Rotate back to 0° (DEFAULT) clockwise around stone[1] (4, -2):
      // [X]
      // [X]
      // [X][X]
      if (this.stones_[0].canMove(1, -1) &&
  ...
  left() {
    if (this.current_rotation_ === ROTATION.DEFAULT) {
      ...
      // Check leftmost stones (stones 2 and 3)
      if (!this.stones_[3].canMove(-1, 0) ||
          !this.stones_[1].canMove(-1, 0) ||
  ...
  right() {
    if (this.current_rotation_ === ROTATION.DEFAULT) {
      // Check rightmost stones (stone 3, 0 and 1)
      if (!this.stones_[3].canMove(1, 0) ||
          !this.stones_[0].canMove(1, 0) ||
          !this.stones_[1].canMove(1, 0)) {
        return false;
