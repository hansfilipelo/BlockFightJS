...
export class TShape {
  constructor(board) {
    ...
    this.color_ = StoneColor.CYAN;

    // T-shape in default orientation (0°):
    //    [X]
    // [X][X][X]
    this.stones_ = [
      new Stone(4, -2, this.color_, this.board_),  // top
      new Stone(3, -1, this.color_, this.board_),  // bottom left
      new Stone(4, -1, this.color_, this.board_),  // bottom center (pivot)
  ...
  drop() {
    if (this.current_rotation_ === ROTATION.DEFAULT) {
      // Check bottom row stones (stones 1, 2, 3)
      ...
      // Check bottom row stones (stones 0 and 2)
      ...
      // Check bottom row stones (stones 1 and 2)
      if (!this.stones_[1].canMove(0, 1) ||
          !this.stones_[0].canMove(0, 1)) {
        return false;
      }
  ...
  rotate() {
    if (this.current_rotation_ === ROTATION.DEFAULT) {
      // Rotate to 90° clockwise around stone[2] (4, -1):
      // [X]
      // [X][X]
      // [X]
      if (this.stones_[3].canMove(-1, 1)) {
        this.stones_[3].move(-1, 1);
        this.stones_[0].move(1, 1);
        this.stones_[1].move(1, -1);
        // stone[2] stays in place (pivot)
        this.current_rotation_ = ROTATION.ROTATED_90;
      }
    } else if (this.current_rotation_ === ROTATION.ROTATED_90) {
      // Rotate to 180° clockwise around stone[2] (4, -1):
      // [X][X][X]
      //    [X]
      let x_pos = this.stones_[3].x_pos();
      if (x_pos === 0) {
        this.right();
      }
      if (this.stones_[3].canMove(-1, -1)) {
      ...
      // Rotate to 270° clockwise around stone[2] (4, -1):
      //    [X]
      // [X][X]
      //    [X]
      if (this.stones_[3].canMove(1, -1)) {
      ...
      // Rotate back to 0° (DEFAULT) clockwise around stone[2] (4, -1):
      //    [X]
      // [X][X][X]
      let x_pos = this.stones_[3].x_pos();
  ...
  left() {
    if (this.current_rotation_ === ROTATION.DEFAULT) {
      // Check leftmost stones (stones 0 and 1)
      ...
      // Check leftmost stones (stones 1 and 2)
      if (!this.stones_[1].canMove(-1, 0) ||
      ...
      // Check leftmost stones (stones 1, 2, 3)
  ...
  right() {
    if (this.current_rotation_ === ROTATION.DEFAULT) {
      // Check rightmost stones (stones 0 and 3)
      if (!this.stones_[0].canMove(1, 0) ||
          !this.stones_[3].canMove(1, 0)) {
        return false;
      }
      ...
      // Check rightmost stones (stones 0, 2, 3)
      if (!this.stones_[0].canMove(1, 0) ||
          !this.stones_[1].canMove(1, 0)) {
        return false;
      }
      ...
      // Check rightmost stones (stones 2 and 3)
      if (!this.stones_[1].canMove(1, 0) ||
          !this.stones_[2].canMove(1, 0) ||
          !this.stones_[3].canMove(1, 0)) {
        return false;
